    ...
    <section className="sponsorship-options" id="propostas">
      ...
      <div className="sponsorship-card-grid sponsorship-card-grid-three">
        <article className="sponsorship-card master-card">
          <p className="card-label">{pt ? "COTA MASTER" : "MASTER TIER"}</p><h3>R$ 100.000</h3>
          <p className="card-summary">{pt ? "Patrocínio integral do valor aprovado, com o mais alto nível de reconhecimento institucional e protagonismo de marca." : "Full sponsorship of the approved amount, with the highest level of institutional recognition and brand prominence."}</p>
          ...
          <a href={`mailto:${email}?subject=Cota Master — The Velvet Margin`}>{pt ? "Conversar sobre a cota Master" : "Discuss the Master tier"}<span>→</span></a>
        </article>
        <article className="sponsorship-card principal-card">
          <p className="card-label">{pt ? "COTA DESTAQUE" : "FEATURED TIER"}</p><h3>R$ 50.000</h3>
          <p className="card-summary">{pt ? "Aporte de destaque para empresas que desejam forte presença institucional em uma composição de patrocinadores." : "A featured contribution for companies seeking strong institutional visibility within a group of sponsors."}</p>
          ...
          <a href={`mailto:${email}?subject=Cota Destaque — The Velvet Margin`}>{pt ? "Conversar sobre a cota Destaque" : "Discuss the Featured tier"}<span>→</span></a>
        </article>
        <article className="sponsorship-card impulso-card">
          <p className="card-label">{pt ? "COTA IMPULSO" : "IMPACT TIER"}</p><h3>R$ 25.000</h3>
          <p className="card-summary">{pt ? "Uma modalidade acessível para empresas que desejam apoiar uma etapa relevante da produção e integrar a rede de patrocinadores do projeto." : "An accessible tier for companies that want to support a meaningful production stage and join the project’s sponsor network."}</p>
          <ul><li>{pt ? "Crédito como Patrocinadora Impulso" : "Impact Sponsor credit"}</li><li>{pt ? "Marca na página de patrocinadores e nos créditos digitais" : "Brand presence on the sponsor page and in digital credits"}</li><li>{pt ? "Menções em publicações selecionadas nas redes sociais" : "Mentions in selected social media posts"}</li><li>{pt ? "Presença nos créditos da Live Session e em materiais digitais selecionados" : "Presence in Live Session credits and selected digital materials"}</li><li>{pt ? "Relatório final com registros e comprovação das entregas" : "Final report with documentation of delivered benefits"}</li></ul>
          <a href={`mailto:${email}?subject=Cota Impulso — The Velvet Margin`}>{pt ? "Conversar sobre a cota Impulso" : "Discuss the Impact tier"}<span>→</span></a>
        </article>
      </div>
      <div className="actions">
