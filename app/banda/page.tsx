...
const artists: Person[] = [
  {
    name: "Arthur Krena", rolePt: "Voz principal", roleEn: "Lead vocals", image: "/members/arthur-krena.webp", className: "portrait-vertical",
  ...
  },
  {
    ...
    bioPt: ["Yves Remont é guitarrista, compositor e professor de música, com trajetória construída na cena musical paulistana e experiência em diferentes vertentes do rock, da música brasileira e da música instrumental. Ao longo de sua carreira, participou de projetos como Maraca Manca, Intocáveis Rock Band, Flamenco Guitar Trio, Projeto Canto para a Lua e Bravecore, acumulando experiência em apresentações, composição, ensino e criação de repertório.", "Integra a Ki’MBANDA, banda de afro rock formada em São Paulo, onde atua como guitarrista e um dos principais compositores. Sua contribuição está diretamente ligada à construção da identidade musical do grupo, que combina a força do rock com referências afro-brasileiras, ancestralidade, afro-samba e afrofuturismo, além da criação de riffs e elementos centrais das composições da banda.", "Em The Velvet Margin, Yves atua na guitarra, trazendo uma experiência marcada pela diversidade de linguagens, pela composição, pelo ensino musical e pela busca de uma identidade própria para o instrumento. Sua trajetória entre rock, música negra e experimentação contribui para a construção das texturas, dos riffs e da linguagem harmônica do projeto."],
    bioEn: ["Yves Remont is a guitarist, composer and music teacher whose career was built within São Paulo’s music scene, with experience across different strands of rock, Brazilian music and instrumental music. Throughout his career, he has taken part in projects such as Maraca Manca, Intocáveis Rock Band, Flamenco Guitar Trio, Projeto Canto para a Lua and Bravecore, developing experience in performance, composition, teaching and repertoire creation.", "He is a member of Ki’MBANDA, an afro-rock band formed in São Paulo, where he works as guitarist and one of the group’s main composers. His contribution is closely connected to the band’s musical identity, combining the power of rock with Afro-Brazilian references, ancestry, afro-samba and Afrofuturism, as well as creating riffs and central elements of the band’s compositions.", "In The Velvet Margin, Yves plays guitar, bringing an approach shaped by diverse musical languages, composition, music education and the search for a distinctive instrumental voice. His path through rock, Black music and experimentation contributes to the project’s textures, riffs and harmonic language."],
  },
  {
  ...
  {
    name: "Anderson Kafé", rolePt: "Percussão", roleEn: "Percussion", image: "/members/anderson-kafe.webp", className: "portrait-vertical",
...
const team: Person[] = [
  {
    name: "Tiago Lins", rolePt: "Proponente, compositor, produtor musical e coordenação executiva", roleEn: "Project lead, songwriter, music producer and executive coordinator", image: "/members/tiago-lins-bio.webp", className: "portrait-vertical",
  ...
  {
    name: "Txai Zerbeto Suares Souza", rolePt: "Direção de arte e design", roleEn: "Art direction and design", image: "/members/txai-zerbeto.webp", className: "portrait-vertical",
  ...
  {
    name: "Dayanne Silva", rolePt: "Coordenação administrativa e direitos autorais", roleEn: "Administrative coordination and copyright", image: "/members/dayanne-silva.webp", className: "portrait-vertical",
...
function PeopleContent() {
  ...
  useEffect(() => {
    ...
    const close = (event: KeyboardEvent) => {
      ...
      const focusable = Array.from(modalRef.current.querySelectorAll<HTMLElement>("button, a[href], [tabindex]:not([tabindex='-1'])"));
    ...
    return () => { document.body.style.overflow = previous; window.removeEventListener("keydown", close); lastTriggerRef.current?.focus(); };
  }, [selected]);
  ...
  return <main className="inner-page">
    ...
    {selected && <div className="bio-modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.currentTarget === event.target) setSelected(null); }}><section ref={modalRef} className="bio-modal" role="dialog" aria-modal="true" aria-labelledby="bio-title"><button ref={closeButtonRef} className="bio-modal-close" type="button" aria-label={pt ? "Fechar biografia" : "Close biography"} onClick={() => setSelected(null)}>×</button><div className={`bio-modal-portrait ${selected.image ? "" : "portrait-placeholder"}`}>{selected.image ? <img src={selected.image} alt="" decoding="async" /> : <div><span>{selected.initials}</span></div>}{(pt ? selected.photoCreditPt : selected.photoCreditEn) && <small className="photo-credit">{pt ? selected.photoCreditPt : selected.photoCreditEn}</small>}</div><div className="bio-modal-copy"><p className="kicker">{pt ? "TRAJETÓRIA" : "BIOGRAPHY"}</p><h2 id="bio-title">{selected.name}</h2><p className="bio-modal-role">{pt ? selected.rolePt : selected.roleEn}</p><div className="bio-modal-text">{(pt ? selected.bioPt : selected.bioEn)?.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}</div></div></section></div>}
  </main>;
}
