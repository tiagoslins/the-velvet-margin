...
function ContactContent() {
  ...
  return <main className="inner-page contact-page contact-page-expanded">
    <section className="contact-copy">
      ...
      <form className="contact-form corporate-contact-form" onSubmit={handleSubmit} aria-busy={sending}>
        <label>{pt ? "Nome" : "Name"}<input name="name" autoComplete="name" maxLength={100} required /></label>
        <label>{pt ? "Empresa" : "Company"}<input name="company" autoComplete="organization" maxLength={120} required /></label>
        <label>{pt ? "Cargo" : "Role"}<input name="role" autoComplete="organization-title" maxLength={120} /></label>
        <label>E-mail {pt ? "corporativo" : "business"}<input name="email" type="email" autoComplete="email" inputMode="email" maxLength={160} required /></label>
        <label>{pt ? "Telefone" : "Phone"}<input name="phone" type="tel" autoComplete="tel" inputMode="tel" maxLength={40} /></label>
        ...
        <label className="full-field privacy-consent"><input name="consent" type="checkbox" required /> <span>{pt ? "Autorizo o uso destes dados exclusivamente para responder a esta solicitação. As informações não serão usadas para listas de divulgação nem compartilhadas com terceiros." : "I authorize the use of this data solely to answer this request. The information will not be used for mailing lists or shared with third parties."}</span></label>
    ...
    <div className="contact-photo contact-photo-full">
      <img src="/contact-band.webp" alt={pt ? "Silhueta de músicos durante uma apresentação" : "Silhouette of musicians performing"} width="816" height="1456" decoding="async" />
    </div>
